...
#### xinference Environment Configuration Guide
...
- Create the chatchat environment
```shell
...
- Create the xinference environment
```shell
...
$ xinference-local
```
- Edit the model registration script
```shell
$ vim model_registrations.sh
# Add the following content. The model path needs to be modified according to your actual situation.
...
- Edit the embedding registration script
```shell
...
```
- Edit the embedding startup script
```shell
...
```
- Initialize the chatchat configuration
```shell
$ conda activate ~/miniconda3/envs/chatchat
