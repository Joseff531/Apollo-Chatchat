...
async def test_mcp_stdio_tools(logging_conf):
    ...
    chat_iterator = agent_executor.invoke(chat_input="Compute 2 times 5, then compute 100*2")
...
async def test_mcp_multi_tools(logging_conf):
    ...
    chat_iterator = agent_executor.invoke(chat_input="Use the browser to download the project locally: https://github.com/microsoft/playwright-mcp")
    async for item in chat_iterator:
        if isinstance(item, PlatformToolsAction):
            print("PlatformToolsAction:" + str(item.to_json()))
...
async def test_mcp_tools(logging_conf):
    ...
    chat_iterator = agent_executor.invoke(chat_input="Compute 2 times 5, then compute 100*2, then fetch the text at this URL https://mp.weixin.qq.com/s/YCHHY6mA8-1o7hbXlyEyEQ, then use the browser to download the project locally: https://github.com/microsoft/playwright-mcp")
    async for item in chat_iterator:
        if isinstance(item, PlatformToolsAction):
            print("PlatformToolsAction:" + str(item.to_json()))
