...
def test_kb_chat_auto():
    data = {
        "messages": [
            {"role": "user", "content": "How can I ask a question to get a high-quality answer?"},
        ],
        "model": "qwen1.5-chat",
...
def test_kb_chat_mannual():
    data = {
        ...
        "model": "qwen1.5-chat",
        "tool_choice": "search_local_knowledgebase",
        "extra_body": {"tool_input": {"database": "samples", "query": "How can I ask a question to get a high-quality answer?"}},
        "stream": True,
    }
    dump_input(data, "KB Chat (auto parameters)")
    response = requests.post(url, headers=headers, json=data, stream=True)
