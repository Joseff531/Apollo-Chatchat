        ...
        try:
            ...
            for elem in root:
                if elem.tag == 'use_mcp_tool':
                    ...
                    # Ignore the thinking tag; it is used for the internal thinking process
                    continue
                elif elem.tag in ['use_mcp_resource']:
                    # Handle the use_mcp_resource tag, skip for now
                    continue
                else:
                    # Handle other tool tags (such as calculate, etc.)
                    tool_name = elem.tag
                    tool_input = {}

                    # Iterate over all child tags, used as tool parameters
                    for child in elem:
                        if child.text and child.text.strip():
                            tool_input[child.tag] = child.text.strip()

                    # Create a generic AgentAction
                    act = AgentAction(
                        tool=tool_name,
