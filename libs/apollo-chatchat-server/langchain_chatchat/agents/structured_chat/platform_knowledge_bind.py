...
def render_knowledge_mcp_tools(tools: List[MCPStructuredTool]) -> str:
    # Use defaultdict to group tools by server_name
    grouped_tools = defaultdict(list)

    for t in tools:
        desc = re.sub(r"\n+", " ", t.description)

        # Build parameter descriptions, emphasizing the required=True attribute
        params = []
        if hasattr(t, "args") and t.args:
            for arg_name, arg_def in t.args.items():
                ...
                # Emphasize the required attribute
                if required:
...
def render_knowledge_tools(tools: List[BaseTool]) -> str:
    ...
    output = []
    for t in tools:
        # Process the description, removing extra line breaks
        desc = re.sub(r"\n+", " ", t.description)

        # Build the parameter section
        params = []
        if hasattr(t, "args") and t.args:  # Ensure parameter definitions exist
            for arg_name, arg_def in t.args.items():
                # Get field information
                required = arg_def.get("required", True)
                required_str = "(required)" if required else "(optional)"
