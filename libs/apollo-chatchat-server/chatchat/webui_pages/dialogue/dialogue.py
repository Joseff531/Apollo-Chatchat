...
def get_messages_history(
    ...
    content_in_expander controls whether content inside expander elements is returned. It is usually enabled when exporting; it is not needed for the history passed to the LLM.
    """
...
def upload_temp_docs(files, _api: ApiRequest) -> str:
    """
    Upload files to a temporary directory for use in file chats.
    Returns the temporary vector store ID.
    """
...
def del_conv(name: str = None):
    ...
    if len(conv_names) == 1:
        sac.alert(
            "Failed to delete conversation", f"This is the last conversation; it cannot be deleted", color="error", closable=True
        )
    elif not name or name not in conv_names:
        sac.alert(
            "Failed to delete conversation", f"Invalid conversation name: \"{name}\"", color="error", closable=True
        )
    else:
...
def dialogue_page(
    ...
    with st.sidebar:
        tab1, tab2 = st.tabs(["Tool Settings", "Conversation Settings"])

        with tab1:
            use_agent = st.checkbox(
                "Enable Agent", help="Please ensure the selected model has Agent capability", key="use_agent"
            )
            ...
            if use_agent:
                use_mcp = st.checkbox("Use MCP", key="use_mcp")
                # selected_tools = sac.checkbox(list(tools), format_func=lambda x: tools[x]["title"], label="Select tools",
                ...
                # selected_tool = sac.buttons(list(tools), format_func=lambda x: tools[x]["title"], label="Select tools",
            ...
            # When Agent is not enabled, manually generate tool parameters
            # TODO: need finer-grained control widgets
            tool_input = {}
            ...
            # Image used for image-based chat and text-to-image
            upload_image = None
    ...
    #     "optional_text_label": "We welcome the reason for your rating",
    # }

    # TODO: this is a bit odd: Settings.model_settings.LLM_MODEL_CONFIG is imported from the backend and then sent from the front end back to the backend. Needs cleanup.
    ...
    if prompt:
        ...
        if len(selected_tools) == 1:
            ...
            tool_choice = None
        # If any field in tool_input is empty, fill it with the user input
        for k in tool_input:
        ...
        params = dict(
            messages=messages,
            model=llm_model,
            stream=stream, # TODO: xinference qwen-vl-chat streaming output fails; revisit after updates
            ...
            try:
                for d in client.chat.completions.create(**params):
                    ...
                    if d.status == AgentStatus.error:
                        ...
                        chat_box.insert_msg("Interpreting tool output...")
                    ...
                    # Tool output duplicates the LLM output
                        ...
                        if d.message_type == MsgType.IMAGE:
                            for url in json.loads(tool_output).get("images", []):
                                # Check whether the URL includes a domain
                                if not url.startswith("http"):
                                    url = f"{api.base_url}/media/{url}"
                                # markdown syntax is unsupported, so pos is skipped
                        ...
                        if getattr(d, "is_ref", False):
                            ...
                            chat_box.insert_msg("")
                        elif getattr(d, "tool_call", None) == "text2images":  # TODO: special handling per tool; a more general approach is needed
