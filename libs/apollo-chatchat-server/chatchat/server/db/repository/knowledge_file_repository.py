...
def list_file_num_docs_id_by_kb_name_and_file_name(
    ...
    """
    List the ids of all Documents that belong to a given file in a given knowledge base.
    Return format: [str, ...]
    """
    doc_ids = (
...
def list_docs_from_db(
    ...
    """
    List all Documents that belong to a given file in a given knowledge base.
...
def delete_docs_from_db(
    ...
    """
    Delete all Documents that belong to a given file in a given knowledge base
    and return the deleted Documents.
    Return format: [{"id": str, "metadata": dict}, ...]
    """
...
def add_docs_to_db(session, kb_name: str, file_name: str, doc_infos: List[Dict]):
    """
    Add the Document information for the given file in the given knowledge base to the database.
    doc_infos format: [{"id": str, "metadata": dict}, ...]
    """
    # ! doc_infos can be None here; this needs further investigation
    if doc_infos is None:
        print(
            "The doc_infos argument passed to server.db.repository.knowledge_file_repository.add_docs_to_db is None"
        )
        return False
    for d in doc_infos:
        obj = FileDocModel(
...
def add_file_to_db(
    ...
    if kb:
        # If the file already exists, update its information and bump the version
