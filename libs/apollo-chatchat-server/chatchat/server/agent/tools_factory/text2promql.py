...
def execute_promql_request(url: str, params: dict, auth: Optional[HTTPBasicAuth], promql: str) -> str:
    try:
        ...
        return f"PromQL: {promql} error: {e}\n"

    if response.status_code == 200:
        return f"PromQL: {promql} query result: {response.json()}\n"
    else:
        return f"PromQL: {promql} error: {response.text}\n"
...
def query_prometheus(query: str, config: dict) -> str:
    ...
    # Build the auth parameter based on whether a username and password are provided.
    ...
    # Send the HTTP request to Prometheus.
...
@regist_tool(title="Prometheus Chat")
def text2promql(
        query: str = Field(
