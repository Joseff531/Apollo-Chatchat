"""
Example usage: python llm_api_stale.py --model-path-address THUDM/chatglm2-6b@localhost@7650 THUDM/chatglm2-6b-32k@localhost@7651
Other fastchat.server.controller/worker/openai_api_server parameters can be passed as documented by fastchat,
but a few non-essential parameters such as --worker-address, --allowed-origins, --allowed-methods, and --allowed-headers are not supported.
...
# 0: log_path
# ! 1: log file name; must match bash_launch_sh
...
def string_args(args, args_list):
    """Convert the keys in args into a string"""
    args_str = ""
    for key, value in args._get_kwargs():
        # Keys from args._get_kwargs use _ as the separator; convert first, then check whether they are in the specified args list
        key = key.replace("_", "-")
        if key not in args_list:
            continue
        # In fastchat, port and host have no prefix; strip the prefix
...
def launch_worker(item, args, worker_args=worker_args):
    ...
    # Split model-path-address first, then pass the pieces to string_args for parsing
    args.model_path, args.worker_host, args.worker_port = item.split("@")
    args.worker_address = f"http://{args.worker_host}:{args.worker_port}"
    print("*" * 80)
    print(f"If it takes a long time to start, please check the log at {LOG_PATH}{log_name}.log")
...
def launch_all(
    ...
    print(f"Starting LLM service; please monitor each module's log under {LOG_PATH}...")
    ...
    print(f"Worker startup time varies by hardware and may take roughly 3-10 minutes; please be patient...")
    if isinstance(args.model_path_address, str):
        ...
        for idx, item in enumerate(args.model_path_address):
            print(f"Starting to load model #{idx}: {item}")
    ...
    print("LLM service has been started.")


if __name__ == "__main__":
    args = parser.parse_args()
    # The http:// prefix is required; otherwise InvalidSchema: No connection adapters were found
