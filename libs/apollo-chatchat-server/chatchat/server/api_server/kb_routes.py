...
@kb_router.post(
    "/{mode}/{param}/chat/completions", summary="Knowledge base chat, OpenAI compatible; parameters match /chat/kb_chat"
)
...
kb_router.post(
    "/create_knowledge_base", response_model=BaseResponse, summary="Create a knowledge base"
)(create_kb)

kb_router.post(
    "/delete_knowledge_base", response_model=BaseResponse, summary="Delete a knowledge base"
)(delete_kb)

kb_router.get(
    "/list_files", response_model=ListResponse, summary="Get the list of files in the knowledge base"
)(list_files)

kb_router.post("/search_docs", response_model=List[dict], summary="Search the knowledge base")(
    search_docs
)
...
kb_router.post(
    "/delete_docs", response_model=BaseResponse, summary="Delete specified files from the knowledge base"
)(delete_docs)

kb_router.post("/update_info", response_model=BaseResponse, summary="Update the knowledge base description")(
    update_info
)

kb_router.post(
    "/update_docs", response_model=BaseResponse, summary="Update existing files in the knowledge base"
)(update_docs)

kb_router.get("/download_doc", summary="Download the corresponding knowledge file")(download_doc)

kb_router.post(
    "/recreate_vector_store", summary="Rebuild the vector store from documents in content; streams the processing progress."
)(recreate_vector_store)

kb_router.post("/upload_temp_docs", summary="Upload files to a temporary directory for use in file chat.")(
    upload_temp_docs
)

kb_router.post("/search_temp_docs", summary="Search the temporary knowledge base")(
    search_temp_docs
)
...
summary_router.post(
    "/summary_file_to_vector_store", summary="Summarize a single knowledge base by file name"
)(summary_file_to_vector_store)
summary_router.post(
    "/summary_doc_ids_to_vector_store",
    summary="Summarize a single knowledge base by doc_ids",
    response_model=BaseResponse,
)(summary_doc_ids_to_vector_store)
summary_router.post("/recreate_summary_vector_store", summary="Rebuild a single knowledge base's file summary")(
    recreate_summary_vector_store
)
