...
# MCP Connection related schemas
class MCPConnectionCreate(BaseModel):
    """Request body for creating an MCP connection"""
    ...
    transport: str = Field(default="stdio", pattern="^(stdio|sse)$", description="Transport type")
...
class MCPConnectionUpdate(BaseModel):
    """Request body for updating an MCP connection"""
    server_name: Optional[str] = Field(None, min_length=1, max_length=100, description="Server name")
    args: Optional[List[str]] = Field(None, description="Command arguments")
    env: Optional[Dict[str, str]] = Field(None, description="Environment variables")
    cwd: Optional[str] = Field(None, description="Working directory")
    transport: Optional[str] = Field(None, pattern="^(stdio|sse)$", description="Transport type")
...
class MCPProfileCreate(BaseModel):
    """Request body for creating MCP general profile"""
...
class MCPProfileResponse(BaseModel):
    """MCP general profile response body"""
    timeout: int
    working_dir: str
...
class MCPProfileStatusResponse(BaseModel):
    """MCP general profile status response body"""
    success: bool
    message: str
