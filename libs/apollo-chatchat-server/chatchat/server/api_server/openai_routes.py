...
DEFAULT_API_CONCURRENCIES = 5  # default maximum concurrency per model
model_semaphores: Dict[
    Tuple[str, str], asyncio.Semaphore
] = {}  # key: (model_name, platform)
openai_router = APIRouter(prefix="/v1", tags=["OpenAI-compatible platform integration endpoints"])


@asynccontextmanager
async def get_model_client(model_name: str) -> AsyncGenerator[AsyncClient]:
    """
    Schedule among models that share the same name, preferring in order:
    an idle model -> the model with the lowest current access count.
    """
    max_semaphore = 0
    selected_platform = ""
...
async def list_models() -> Dict:
    """
    Aggregate the model lists from all platforms.
    """
...
@openai_router.post("/audio/translations", deprecated="Not currently supported")
...
@openai_router.post("/audio/transcriptions", deprecated="Not currently supported")
...
@openai_router.post("/audio/speech", deprecated="Not currently supported")
