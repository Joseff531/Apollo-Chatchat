...
class RapidOCRPDFLoader(UnstructuredFileLoader):
    def _get_elements(self) -> List:
        def rotate_img(img, angle):
            ...
            # Arg 1: rotation centre point.
            # Arg 2: rotation angle. Positive values rotate counter-clockwise; negative values rotate clockwise.
            # Arg 3: isotropic scale factor. 1.0 keeps the original size, 2.0 doubles it, 0.5 halves it.
        ...
        def pdf2text(filepath):
            import fitz  # The fitz module ships with PyMuPDF; do not confuse it with `pip install fitz`
            import numpy as np
            ...
            for i, page in enumerate(doc):
                ...
                for img in img_list:
                    if xref := img.get("xref"):
                        bbox = img["bbox"]
                        # Check whether the image size exceeds the configured threshold
                        ...
                        samples = pix.samples
                        if int(page.rotation) != 0:  # If the page has a rotation angle, rotate the image accordingly
