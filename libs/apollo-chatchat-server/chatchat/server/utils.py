...
def get_base_url(url):
    ...
    base_url = '{uri.scheme}://{uri.netloc}/'.format(uri=parsed_url)  # Format the base URL
    return base_url.rstrip('/')


def get_config_platforms() -> Dict[str, Dict]:
    """
    Get the configured model platforms; pydantic models are converted to dictionaries.
    """
...
def get_config_models(
    ...
    for m in list(get_config_platforms().values()):
        ...
        if m.get("auto_detect_model"):
            if not m.get("platform_type") == "xinference":  # TODO: currently only xf supports auto-detecting models
...
def get_model_info(
    ...
    """
    Get the configured model info, primarily api_base_url and api_key.
    If multiple=True is specified, return all models that share the same name; otherwise return only the first one.
    """
...
def get_OpenAI(
    ...
    # TODO: fetch model info from the API
    model_info = get_model_info(model_name)
    params = dict(
        streaming=streaming,
        verbose=verbose,
...
class ChatMessage(BaseModel):
    ...
    class Config:
        json_schema_extra = {
            "example": {
                "question": "How do I apply for work-injury insurance?",
                "response": "Based on the known information, it can be summarized as follows:\n\n1. The participating employer pays work-injury insurance premiums for its employees to ensure they can receive the corresponding benefits when a work-related injury occurs.\n"
                            "2. Work-injury insurance contribution rules may vary by region. Consult your local social-security department for the specific contribution standards and regulations.\n"
                            "3. Injured workers and their close relatives must apply for a work-injury determination, confirm benefit eligibility, and make timely premium payments.\n"
                            "4. Work-injury insurance benefits include medical treatment for work injuries, rehabilitation, assistive-device costs, disability benefits, death-from-work benefits, and lump-sum death benefits.\n"
                            "5. Eligibility verification for receiving work-injury benefits includes verification for recipients of long-term benefits and recipients of lump-sum benefits.\n"
                            "6. Items paid by the work-injury insurance fund include medical treatment benefits, rehabilitation benefits, assistive-device costs, lump-sum death benefits, funeral subsidies, and more.",
                "history": [
                    [
                        "What is work-injury insurance?",
                        "Work-injury insurance is a social insurance system in which an employer, in accordance with national regulations, pays work-injury insurance premiums on behalf of its own employees and other personnel of the employer, "
                        "and the insurance authority grants work-injury insurance benefits at the standards prescribed by the state.",
                    ]
                ],
                "source_documents": [
                    "Source [1] Guidelines on Specific Personnel Employed by Guangzhou Entities Joining Work-Injury Insurance.docx:\n\n\t"
                    "(1) Employing entities (organizations) shall, under the principle of 'voluntary enrollment', enroll specific employees who have not established a labor relationship in work-injury insurance on a single-item basis and pay the corresponding premiums.",
                    "Source [2] ...",
                    "Source [3] ...",
                ],
            }
...
def run_async(cor):
    """
    Run async code from a synchronous environment.
    """
    try:
...
def iter_over_async(ait, loop=None):
    """
    Wrap an async generator as a synchronous generator.
    """
    ait = ait.__aiter__()
...
# Retrieve model info from model_config
# TODO: After moving the model loading out, these helpers need to be removed or reimplemented
...
#     if path_str := paths.get(model_name):  # For example, "chatglm-6b": "THUDM/chatglm-6b-new"; all of the paths below are supported
#         path = Path(path_str)
...
def api_address(is_public: bool = False) -> str:
    '''
    Allows users to configure public_host and public_port under basic_settings.API_SERVER
    so that when using a cloud server or reverse proxy, the correct public API address can be
    generated (for example, knowledge base document download links).
    '''
...
def get_prompt_template(type: str, name: str) -> Optional[str]:
    """
    Load the template content from prompt_config.
    type: must be one of the categories in model_settings.llm_model_config, or "rag". New features should be added here as well.
    """
...
def get_prompt_template_dict(type: str, name: str) -> Optional[Dict]:
    ...
    Load the template content from prompt_config.
    type: must be one of the categories in model_settings.llm_model_config, or "rag". New features should be added here as well.
    Returns: the defined object dictionary with keys "SYSTEM_PROMPT" and "HUMAN_MESSAGE".
    """
...
def set_httpx_config(
    ...
    """
    Set the default httpx timeout. The httpx default of 5 seconds is too short for LLM requests.
    Add this project's related services to the no-proxy list to avoid fastchat server request errors. (No effect on Windows.)
    For online APIs such as ChatGPT, you must configure a proxy manually. How to handle search-engine proxies is still TBD.
    """
    ...
    # Set process-wide system-level proxy
    proxies = {}
...
def run_in_thread_pool(
    ...
    """
    Run tasks in batch in a thread pool and yield the results via a generator.
    Make sure every operation in the task is thread-safe, and have the task function take only keyword arguments.
    """
    tasks = []
...
def run_in_process_pool(
    ...
    """
    Run tasks in batch in a process pool and yield the results via a generator.
    Make sure every operation in the task is process-safe, and have the task function take only keyword arguments.
    """
    tasks = []
    max_workers = None
...
def get_server_configs() -> Dict:
    """
    Get the raw configuration items from configs for the frontend to use.
    """
    _custom = {
...
# Dynamically refresh the knowledge base information
