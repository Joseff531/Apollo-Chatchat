...
class ThreadSafeFaiss(ThreadSafeObject):
    ...
    def save(self, path: str, create_path: bool = True):
        with self.acquire():
            ...
            logger.info(f"Vector store {self.key} has been saved to disk")
        return ret
    ...
    def clear(self):
        ...
        with self.acquire():
            ...
            logger.info(f"Vector store {self.key} has been cleared")
        return ret
...
class _FaissPool(CachePool):
    ...
    def unload_vector_store(self, kb_name: str):
        if cache := self.get(kb_name):
            self.pop(kb_name)
            logger.info(f"Successfully released vector store: {kb_name}")
...
class KBFaissPool(_FaissPool):
    def load_vector_store(
        ...
        cache = self.get((kb_name, vector_name))  # using a tuple is better than concatenating strings
        try:
            if cache is None:
            ...
            if locked:  # we don't know exception raised before or after atomic.release
                self.atomic.release()
            logger.exception(e)
            raise RuntimeError(f"Failed to load vector store {kb_name}.")
        return self.get((kb_name, vector_name))


class MemoFaissPool(_FaissPool):
    r"""
    Cache pool for temporary vector stores
    """
