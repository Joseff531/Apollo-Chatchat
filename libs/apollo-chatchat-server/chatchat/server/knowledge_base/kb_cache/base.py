...
class ThreadSafeObject:
    ...
    def acquire(self, owner: str = "", msg: str = "") -> Generator[None, None, FAISS]:
        ...
        try:
            ...
            logger.debug(f"{owner} starting operation: {self.key}. {msg}")
            yield self._obj
        finally:
            logger.debug(f"{owner} finished operation: {self.key}. {msg}")
            self._lock.release()
...
class CachePool:
    ...
    def acquire(self, key: Union[str, Tuple], owner: str = "", msg: str = ""):
        cache = self.get(key)
        if cache is None:
            raise RuntimeError(f"Requested resource {key} does not exist")
