    ...
    def do_drop_kb(self):
        with Session(PGKBService.engine) as session:
            session.execute(
                text(
                    f"""
                    -- Delete records from langchain_pg_embedding associated with langchain_pg_collection
                    DELETE FROM langchain_pg_embedding
                    WHERE collection_id IN (
                      SELECT uuid FROM langchain_pg_collection WHERE name = '{self.kb_name}'
                    );
                    -- Delete records from langchain_pg_collection
...
if __name__ == "__main__":
    ...
    # print(pGKBService.search_docs("How to start the api service"))
