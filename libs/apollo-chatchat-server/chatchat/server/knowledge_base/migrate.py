...
def import_from_db(
    ...
    """
    Import data from a backup database into info.db when the knowledge base and vector store are unchanged.
    This applies when info.db's structure changes during a version upgrade but re-vectorization is not required.
    Make sure the table names on both sides match and the field names to be imported match as well.
...
def folder2db(
    ...
    def files2vs(kb_name: str, kb_files: List[KnowledgeFile]) -> List:
        ...
        for success, res in files2docs_in_thread(
            ...
            if success:
                _, filename, docs = res
                print(
                    f"Adding {kb_name}/{filename} to the vector store, containing {len(docs)} documents"
                )
    ...
    for kb_name in kb_names:
        ...
        # Clear the vector store and rebuild from local files
        if mode == "recreate_vs":
            kb.clear_vs()
            kb.create_kb()
        ...
        # # Do not vectorize file content; only store file metadata in the database
        # # Since the database now stores a lot of text-splitting-related information,
        # # simply storing file metadata is not meaningful, so this feature is removed.
        # elif mode == "fill_info_only":
        #     files = list_files_from_folder(kb_name)
        #     kb_files = file_to_kbfile(kb_name, files)
        #     for kb_file in kb_files:
        #         add_file_to_db(kb_file)
        #         print(f"Added {kb_name}/{kb_file.filename} to the database")
        # Using the file list in the database as the baseline, update the vector store from local files
        elif mode == "update_in_db":
            files = kb.list_files()
            kb_files = file_to_kbfile(kb_name, files)
            result = files2vs(kb_name, kb_files)
            kb.save_vector_store()
        # Compare the local directory with the file list in the database and perform incremental vectorization
        elif mode == "increment":
        ...
        kb_path = (
            f"Knowledge base path\t: {kb.kb_path}\n"
        ...
        print(
            (
                f"Knowledge base name\t: {kb_name}\n"
                f"Knowledge base type\t: {kb.vs_type()}\n"
                f"Embedding model\t: {kb.embed_model}\n"
            )
            + kb_path
            + (
                f"Total file count\t: {file_count}\n"
                f"Files ingested\t: {success_count}\n"
                f"Knowledge entries\t: {docs_count}\n"
