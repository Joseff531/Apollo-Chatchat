...
def validate_kb_name(knowledge_base_id: str) -> bool:
    # Check whether it contains unexpected characters or path-attack keywords
...
def list_files_from_folder(kb_name: str):
    ...
    def process_entry(entry):
        ...
        if entry.is_symlink():
            ...
            ).as_posix()  # Normalize the path to posix format
...
LOADER_DICT = {
    ...
    "CSVLoader": [".csv"],
    # "FilteredCSVLoader": [".csv"], if using a custom CSV splitter
...
def get_loader(loader_name: str, file_path: str, loader_kwargs: Dict = None):
    """
    Return a document loader based on the loader_name and file path or content.
    """
    ...
    if loader_name == "UnstructuredFileLoader":
        ...
        if not loader_kwargs.get("encoding"):
            # If encoding is not specified, automatically detect the file encoding to avoid encoding errors when the langchain loader loads the file
...
def make_text_splitter(splitter_name, chunk_size, chunk_overlap):
    ...
    try:
        if (
        ...
        ):  # special handling for MarkdownHeaderTextSplitter
            ...
            try:  # prefer the user's custom text_splitter
...
class KnowledgeFile:
    def __init__(
        ...
        """
        Corresponds to a file in the knowledge base directory; the file must exist on disk to allow vectorization or other operations.
        """
...
def files2docs_in_thread(
    ...
    """
    Convert disk files to langchain Documents in batch using multiple threads.
    If the input is a Tuple, the form is (filename, kb_name).
    The generator yields status, (kb_name, file_name, docs | error).
    """
...
def format_reference(kb_name: str, docs: List[Dict], api_base_url: str="") -> List[Dict]:
    '''
    Format knowledge base search results as reference documents
    '''
    ...
    for inum, doc in enumerate(docs):
        ...
        ref = f"""Source [{inum + 1}] [{filename}]({url}) \n\n{page_content}\n\n"""
